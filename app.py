...
if not st.session_state.authenticated:
    ...
    with login_tab:
        with st.form("login_form"):
            ...
            submitted = st.form_submit_button(lang_manager.get_text("login_button", lang), use_container_width=True)
    ...
    with register_tab:
        with st.form("register_form"):
            ...
            submitted = st.form_submit_button(lang_manager.get_text("register_button", lang), use_container_width=True)
...
with st.sidebar:
    ...
    if st.button("🔄 Refresh Data / 刷新數據", use_container_width=True):
        st.session_state.trades_df = st.session_state.db.get_trades()
        st.rerun()
    
    if st.button("📊 Add Sample Data / 添加示例數據", use_container_width=True):
        create_sample_data()
        st.session_state.trades_df = st.session_state.db.get_trades()
        st.success("Sample data added! / 示例數據已添加！")
        st.rerun()
    
    if st.button("🗑️ Reset Database / 重置數據庫", use_container_width=True):
...
with tab2:
    ...
    if asset_type == "📈 Stock":
        ...
        with st.form("stock_trade_form"):
            ...
            submitted = st.form_submit_button("📝 Submit Stock Trade", use_container_width=True)
    ...
    # Options Trading Section
        ...
        with st.form("options_trade_form"):
            ...
            submitted = st.form_submit_button("🎯 Submit Options Trade", use_container_width=True)
    ...
    # Crypto Trading Section
        ...
        with st.form("crypto_trade_form"):
            ...
            submitted = st.form_submit_button("₿ Submit Crypto Trade", use_container_width=True)
    ...
    # Other asset types (simplified)
        ...
        with st.form(f"{asset_type.lower().replace(' ', '_')}_trade_form"):
            ...
            submitted = st.form_submit_button(f"📝 Submit {asset_type} Trade", use_container_width=True)
...
with tab5:
    ...
    if st.session_state.trades_df.empty:
        ...
        daily_pnl.columns = ['Date', 'Realized P&L', 'Unrealized P&L', 'Total Cost', 'Trade Count', 'Total P&L']
        ...
        if not daily_pnl.empty:
            ...
            daily_pnl['Date'] = daily_pnl['Date'].dt.strftime('%Y-%m-%d')
            daily_pnl['Realized P&L'] = daily_pnl['Realized P&L'].apply(lambda x: f"${x:.2f}")
            daily_pnl['Unrealized P&L'] = daily_pnl['Unrealized P&L'].apply(lambda x: f"${x:.2f}")
            daily_pnl['Total P&L'] = daily_pnl['Total P&L'].apply(lambda x: f"${x:.2f}")
            daily_pnl['Total Cost'] = daily_pnl['Total Cost'].apply(lambda x: f"${x:.2f}")
            ...
            with col2:
                total_realized = daily_pnl['Realized P&L'].str.replace('$', '').astype(float).sum()
                st.metric("Total Realized P&L", f"${total_realized:.2f}")
            
            with col3:
                total_unrealized = daily_pnl['Unrealized P&L'].str.replace('$', '').astype(float).sum()
            ...
            with col1:
                best_day = daily_pnl.loc[daily_pnl['Total P&L'].str.replace('$', '').astype(float).idxmax()]
                st.success(f"**Best Day:** {best_day['Date']} - {best_day['Total P&L']}")
            
            with col2:
                worst_day = daily_pnl.loc[daily_pnl['Total P&L'].str.replace('$', '').astype(float).idxmin()]
                st.error(f"**Worst Day:** {worst_day['Date']} - {worst_day['Total P&L']}")
            
            # Create a simple calendar heatmap
            st.markdown("### 📅 P&L Heatmap")
            
            # Create a simple heatmap using st.bar_chart
            daily_pnl['Total P&L Numeric'] = daily_pnl['Total P&L'].str.replace('$', '').astype(float)
            chart_data = daily_pnl.set_index('Date')['Total P&L Numeric']
